...
async function autoRegisterCustomPools(
  ...
  // Custom token pools on testnet (FIXED - from FIXED_LIQUIDITY_POOLS.env)
  const customPools = [
    { token: 'CAABHEKIZJ3ZKVLTI63LHEZNQATLIZHSZAIGSKTAOBWGGINONRUUBIF3', pool: 'CD7U5F4EKUC7UM72F3D5G4UPS6DJ54RNNPSHKJBK7KCRX4N3MNNBDGES' }, // AQX (FIXED)
    { token: 'CBBBGORMTQ4B2DULIT3GG2GOQ5VZ724M652JYIDHNDWVUC76242VINME', pool: 'CCPV4CJUVHJ7DHN7UTHXORHOBVBP7ALNYU5QBXA6MJBFVS2Z5JWQHJRC' }, // VLTK (FIXED)
    { token: 'CCU7FIONTYIEZK2VWF4IBRHGWQ6ZN2UYIL6A4NKFCG32A2JUEWN2LPY5', pool: 'CBIM5CPY2T3KM6BDH6KVQNTPZ244VF6SJYB6DHB25UU6XGUWDNGICQMA' }, // SLX (FIXED)
    { token: 'CCAIKLYMECH7RTVNR3GLWDU77WHOEDUKRVFLYMDXJDA7CX74VX6SRXWE', pool: 'CCYBUIBFOWROWGLZWJWWVOWJSDVYF4XX3PW6U7Z4SWZVG64LPTWUT3S4' }, // WRX (FIXED)
    { token: 'CDYGMXR7K4DSN4SE4YAIGBZDP7GHSPP7DADUBHLO3VPQEHHCDJRNWU6O', pool: 'CCOQIJJM6VWFH6YA4QU2D3GX5YUEJOWDLZP4EPTIIL5AFLMRZKTBNRJW' }, // SIXN (FIXED)
    { token: 'CBXSQDQUYGJ7TDXPJTVISXYRMJG4IPLGN22NTLXX27Y2TPXA5LZUHQDP', pool: 'CBERM7NGHSHHKDNOFE7DAMYLHYKK2M5JJBP4QSYL65OMSYGKBN3M2GUE' }, // MBIUS (FIXED)
    { token: 'CB4MYY4N7IPH76XX6HFJNKPNORSDFMWBL4ZWDJ4DX73GK4G2KPSRLBGL', pool: 'CBP6CNZQIJHP66L6NY7YJVZ4GBC773XKNRLG4TMD7D773HQ5LZ46OEZ4' }, // TRIO (FIXED)
    { token: 'CDRFQC4J5ZRAYZQUUSTS3KGDMJ35RWAOITXGHQGRXDVRJACMXB32XF7H', pool: 'CDPGBKILVQXIDSVOUHQ4YOK5P7PNZVPSVM7GNSJX3JI5NQRNTSGOKEGN' }, // RELIO (FIXED)
    { token: 'CB4JLZSNRR37UQMFZITKTFMQYG7LJR3JHJXKITXEVDFXRQTFYLFKLEDW', pool: 'CAQ2366OMMX74H7QB4MQTEMH3RVSLNU4O5Y2HMJ5AWT2M4ONZR7IE47Y' }, // TRI (FIXED)
    { token: 'CDBBFLGF35YDKD3VXFB7QGZOJFYZ4I2V2BE3NB766D5BUDFCRVUB7MRR', pool: 'CA6KURSAHVBAOZXPD3M5CQLWNDCJ6IWEH2A3XCCVPAXI2DDVVBESAVB5' }, // NUMER (FIXED)
  ];
  ...
  try {
    ...
    const sanitizedConfig = shouldIncludeStrategy ? vault.config : {
      assets: vault.config?.assets?.map((a: any) => {
        // Handle both string assets ("XLM") and object assets ({ code: "XLM", allocation: 100 })
        if (typeof a === 'string') {
          return a;
        }
        // Keep allocation info as it's not sensitive, just public vault strategy
        return {
          code: a.code || a.assetCode || a,
          allocation: a.allocation || 0,
        };
      }) || [],
      isPublic: vault.config?.isPublic,
      rules: vault.config?.rules, // Include rules so we can extract target_allocation
      // Hide visual builder internals like nodes, edges, etc.
    };
